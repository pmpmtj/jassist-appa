"""
Calendar module adapter for OpenAI Assistant Client.

This module provides a specialized interface for calendar processing
...
logger = setup_logger("calendar_adapter", module="api_assistants_cliente")

class CalendarAssistantAdapter:
    """
    Adapter for using the OpenAI Assistant Client with calendar processing.
    """
    ...
    def __init__(
        ...
        """
        Initialize the calendar assistant adapter.
        ...
        self.module_name = "calendar"
        
        # Create or use the provided client
        if client:
            self.client = client
        else:
            # Default config file path - use calendar_assistant_config.json in the calendar module
            if config_file is None:
                voice_diary_dir = Path(__file__).resolve().parent.parent.parent.parent
                config_file = voice_diary_dir / "voice_diary" / "calendar" / "config" / "calendar_assistant_config.json"
                logger.info(f"Using calendar module config file: {config_file}")
            ...
            # Create client with calendar-specific settings
        ...
        if prompts_file:
            ...
            logger.info(f"Voice diary directory: {voice_diary_dir}")
            
            # Construct path to the calendar module's prompts file in the voice_diary directory
            prompts_path = voice_diary_dir / "voice_diary" / "calendar" / "config" / "prompts.yaml"
    ...
    def process_calendar_entry(self, entry_content: str) -> str:
        """
        Process a calendar entry using the OpenAI assistant.
        ...
        try:
            ...
            error_msg = f"Error processing calendar entry: {e}"
            logger.error(error_msg)
            raise AssistantClientError(error_msg)


def process_with_calendar_assistant(entry_content: str) -> str:
    """
    Process a calendar entry using a calendar assistant.
    
    This function provides a simple interface to process calendar entries
    ...
    Raises:
        ...
        AssistantClientError: If processing fails
    """
    adapter = CalendarAssistantAdapter()
    return adapter.process_calendar_entry(entry_content) 