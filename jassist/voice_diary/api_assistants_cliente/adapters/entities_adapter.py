...
Adapter for processing entity information with OpenAI Assistants.
"""
...
logger = setup_logger("entities_adapter", module="api_assistants_cliente")
...
def load_prompts() -> Dict[str, Any]:
    ...
    """
    try:
        module_dir = get_module_dir("entities")
...
def get_entities_assistant_client() -> OpenAIAssistantClient:
    """
    Get a configured OpenAI Assistant client for entities processing.
    
    Returns:
        OpenAIAssistantClient: Configured client
    """
    # Load the config for entities assistant
    config = get_module_config("entities", "entities_assistant_config.json")
...
def process_with_entities_assistant(text: str) -> str:
    """
    Process text with the entities assistant to extract entity information.
    
    Args:
        text: Text to process for entity information
        
    Returns:
        str: JSON-structured response from the assistant
    """
    # Get the client
    client = get_entities_assistant_client()
    ...
    if parse_prompt:
        ...
        template = client.config.get("prompt_template", 
            ...
            "name, type, context, and relevance_score (float between 0-1). " +
            "If information for a field is not available, use an empty string or appropriate default value. " +
